...
Pct_CollegeEd_tract <- Pct_CollegeEd_tract  %>% mutate(CensusTractID = as.character(GEOID)) %>% dplyr::select(-GEOID)
...
SpatialVars <- inner_join(ParkCoverage, DistToDowntown1, by = "CensusTractID") %>% inner_join(., Census_TransitCoverage, by = "CensusTractID") %>% dplyr::select(-variable)
